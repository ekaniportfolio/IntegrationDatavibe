...
const TAB_TITLE_KEYS: Record<string, string> = {
    ...
    countries: "dashboard.tabCountries",
    strategy: "dashboard.tabFMStations",
    default: "dashboard.tabDashboard"
};

interface RadioDashboardHeaderProps {
  activeTab?: string;
  direction?: number;
}

export default function RadioDashboardHeader({ activeTab = "dashboard", direction = 0 }: RadioDashboardHeaderProps) {
  ...
  return (
    <div className="flex flex-col gap-[2px] px-1 w-full relative overflow-hidden items-center">
      <div className="w-full">
         <h2 className="font-manrope font-bold text-[22px] text-white text-center leading-tight">{t("dashboard.sectionMedia")}</h2>
      </div>
